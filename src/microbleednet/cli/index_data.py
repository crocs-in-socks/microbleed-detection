...
def validate_pattern(value: str) -> str:
    ...
    if "{subject_id}" not in value:
        raise typer.BadParameter(
            "The input must contain the '{subject_id}' placeholder."
        )
    return value
...
@app.command(
    name="index-data",
    help="""
        Generates a dataset directory containing manifests that list the
        subjects included in the dataset. If masks are provided, they are
        matched to the volumes using subject ids extracted from the file names
...
def validate_parameters(
    dataset_dir: Annotated[
        Path, typer.Option(help="Directory to which the indexed dataset will be saved.")
    ],
    input_dir: Annotated[
        Path,
        typer.Option(
    ...
    label_dir: Annotated[
        Optional[Path],
        typer.Option(
            help="Label directory containing masks to be indexed.", exists=True
        ),
    ] = None,
    mask_pattern: Annotated[
        Optional[str],
        typer.Option(
            help=(
    ...
    if label_dir is not None and mask_pattern is None:
        raise typer.BadParameter(
            "If you provider --label-dir, you MUST also provide --mask-pattern"
        )
