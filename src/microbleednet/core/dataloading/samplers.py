...
class EqualBatchSampler(Sampler):
    def __init__(
        ...
        if batch_size < 2 or batch_size % 2:
            raise ValueError(
                "balanced batch_size must be an even integer of at least two"
            )
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            positive = torch.randint(
                len(self.pos_indices), (self.n_pos,), generator=self.generator
            )
            negative = torch.randint(
                len(self.neg_indices), (self.n_neg,), generator=self.generator
            )
