...
def match_components(prediction_mask: np.ndarray, reference_mask: np.ndarray) -> dict[str, Any]:
    ...
    if prediction_mask.shape != reference_mask.shape or prediction_mask.ndim != 3:
        raise ValueError("prediction_mask and reference_mask must be matching 3D arrays")

    prediction_labels = label(prediction_mask > 0, connectivity=3)
    reference_labels = label(reference_mask > 0, connectivity=3)
    ...
    if prediction_count and reference_count:
        ...
        np.add.at(
            overlap,
            (prediction_labels[overlapping_voxels] - 1, reference_labels[overlapping_voxels] - 1),
            1,
        )
