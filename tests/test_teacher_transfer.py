...
def test_teacher_transfer_updates_all_parameter_groups() -> None:
    ...
    assert any(
        name.startswith("feature_extractor.") and value
        for name, value in changed.items()
    )
    assert any(
        name.startswith("segmentor.") and value for name, value in changed.items()
    )
    assert any(
        name.startswith("classifier.") and value for name, value in changed.items()
    )
